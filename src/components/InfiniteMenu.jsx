...
function createShader(gl, type, source) {
  ...
  if (!shader) {
    console.error('InfiniteMenu: unable to create shader. WebGL context may be unavailable.')
    return null
  }
  ...
  console.error(`InfiniteMenu ${shaderType} shader compile failed:`, info)
...
function createProgram(gl, shaderSources, transformFeedbackVaryings, attribLocations) {
  ...
  if (!program) {
    console.error('InfiniteMenu: unable to create WebGL program.')
    return null
  }

  const shaders = [gl.VERTEX_SHADER, gl.FRAGMENT_SHADER].map((type, ndx) =>
    createShader(gl, type, shaderSources[ndx]),
  )
  ...
  console.error('InfiniteMenu shader program link failed:', gl.getProgramInfoLog(program) || 'Unknown link error')
  gl.deleteProgram(program)
  return null
}
  ...
  update(deltaTime, targetFrameDuration = 16) {
    ...
    if (this.isPointerDown) {
      ...
      if (this.snapTargetDirection) {
        const snapIntensity = 0.2
        const a = this.snapTargetDirection
        const b = this.snapDirection
        const sqrDist = vec3.squaredDistance(a, b)
        const distanceFactor = Math.max(0.1, 1 - sqrDist * 10)
        angleFactor *= snapIntensity * distanceFactor
...
class InfiniteGridMenu {
  constructor(canvas, items, onActiveItemChange, onMovementChange, onInit = null, scale = 1) {
    ...
    this.autoRotateAxis = vec3.fromValues(0, 1, 0)
  ...
  _init(onInit) {
    ...
    if (!this.discProgram) {
      throw new Error('InfiniteMenu: failed to initialize shader program')
    }
  ...
  _animate(deltaTime) {
    ...
    if (this.autoRotate && !this.control.isPointerDown) {
      const autoRotation = quat.setAxisAngle(
        quat.create(),
        this.autoRotateAxis,
        this.autoRotateSpeed * (deltaTime * 0.001),
      )
      this.control.orientation = quat.multiply(quat.create(), autoRotation, this.control.orientation)
